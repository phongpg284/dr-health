...
export const Game_3: React.FC<Game_3Props> = ({ id, play }) => {
    ...
    React.useEffect(() => {
        if (game && game.getGame && game.getGame.imgPath[0]) {
            setImage(REACT_APP_GAME_FILES_URL + "/" + game.getGame.patientId + "/" + id + "/" + game.getGame.imgName[0])
        }
        gameApply()
        setShouldPlay(1)
    }, [game])
    ...
    const [levelNum, setLevelNum] = React.useState(0) // number of levels to play (MUST SAME LENGTH OF LIST)
    ...
    const BoxElement: React.FC<BoxElementProps> = ({ numberE }) => {
        function clickBox(e: React.MouseEvent) {
            if (shouldPlay == 1) {
                const allElement = document.querySelectorAll(`.game_3_item`)
        ...
        return <>
            {isTrueElement.map((element, i) => {
                if (element) {
                    return <div className={`game_3_item game_3_item_true ${isPicked[i] ? "game_3_item_picked" : ""} ${shouldPlay == 0 ? "minigame_hidden" : ""}`} key={i} onClick={clickBox}>
                        {ans[i]}
                    </div>
                } else {
                    return <div className={`game_3_item ${isPicked[i] ? "game_3_item_picked" : ""} ${shouldPlay == 0 ? "minigame_hidden" : ""}`} key={i} onClick={clickBox}>
                        {ans[i]}
                    </div>
                }
            })}
    ...
    return (<>
        ...
        <div className="game_content">
            ...
            <div className="minigame_image">
                <Image className="minigame_imgsrc" src={image} />
            </div>
            <div className="minigame_row minigame_game_3 mg_wrap mg_juround">
